...
class LevelFactory {
	...
	constructor(scene, camera, resourceHandler, onFinished) {
		this._scene = scene;
		this._camera = camera;
	...
	stringToLevelObject(levelString) {
		var levelObject = new Level(this._scene, this._camera, this._resourceHandler, this._onFinished);
		...
		// Spawn guy
		levelObject._guy = new Guy(levelJSON.guy._posX, levelJSON.guy._posY, levelObject._scene, levelObject._resourceHandler);
		...
		for (var i = 0; i < ps.length; i++) {
			levelObject._platforms.push( new Platform(ps[i]._width, ps[i]._height,
											...
											levelObject._scene, levelObject._resourceHandler) );
		}
		...
		levelObject._movablePlatform = new MovablePlatform(movPlat._width, movPlat._height,
										...
										levelObject._scene, levelObject, levelObject._resourceHandler);
		...
		if (levelJSON.boxes) {
			for (var i = 0; i < levelJSON.boxes.length; i++) {
				...
				levelObject._boxes.push( new Box(box._width, box._height, box._posX, box._posY, CONS_BOX_DEFAULT_MASS, levelObject._scene, levelObject._resourceHandler));
			}
		}
			...
			for (var i = 0; i < levelJSON.emitters.length; i++) {
				...
				levelObject._emitters.push(
					new Emitter(emitter._posX, emitter._posY,
						...
						levelObject._scene, levelObject._resourceHandler, levelObject)
				);
			}
		}

		// Set finish
		levelObject._finish = new Finish(levelJSON.finish._posX, levelJSON.finish._posY, levelObject._scene, levelObject._resourceHandler);
